...
namespace CreditPincher.Tests
{
    public class CreditFormatterTests
    {
        public CreditFormatterTests()
        {
            ...
            // .NET Framework has no settable CultureInfo.CurrentCulture, so this goes
            // through the current thread instead.
        ...
        {
            var credits = new CreditFormatter(false);
            var dollars = new CreditFormatter(true, 100.0);
        ...
        {
            var formatter = new CreditFormatter(true, 250.0);
        ...
        {
            var formatter = new CreditFormatter(true, 0);
        ...
        public void KeepsTheTrayTooltipWithinTheWindowsLimit()
        {
            var formatter = new CreditFormatter();
            var stats = CreditStatsCalculator.Calculate(
                new[] { new CreditUsageEntry(DateTimeOffset.Parse("2026-07-01T09:00:00Z"), 123456789.0) },
                new DateOnly(2026, 7, 1),
                new DateOnly(2026, 7, 31),
                310.0,
                TimeZoneInfo.Utc);
