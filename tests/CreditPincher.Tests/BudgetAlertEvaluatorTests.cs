...
namespace CreditPincher.Tests
{
    ...
    {
        private static readonly YearMonth July = new YearMonth(2026, 7);

        [Fact]
        public void FiresOnceWhenAThresholdIsCrossed()
        {
            var settings = new AppSettings { BudgetNotificationThresholds = new List<int> { 80, 100 } };
            var stats = StatsWith(250.0, 300.0);
        ...
        {
            var settings = new AppSettings { BudgetNotificationThresholds = new List<int> { 50, 80, 100 } };
            var stats = StatsWith(330.0, 300.0);
        ...
        {
            var settings = new AppSettings { BudgetNotificationThresholds = new List<int> { 80 } };
            var stats = StatsWith(250.0, 300.0);
        ...
        public void StaysQuietWithoutABudgetOrWhenDisabled()
        {
            ...
            Assert.Null(BudgetAlertEvaluator.Evaluate(settings, StatsWith(250.0, null), July));

            settings.NotifyOnBudgetThresholds = false;
            Assert.Null(BudgetAlertEvaluator.Evaluate(settings, StatsWith(250.0, 300.0), July));
        }

        private static UsageStats StatsWith(double totalCredits, double? budget)
        {
