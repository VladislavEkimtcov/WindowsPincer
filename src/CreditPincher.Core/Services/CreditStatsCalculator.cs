...
public static class CreditStatsCalculator
{
        ...
        TimeZoneInfo? zone = null)
    {
        ...
        var filtered = entries
            .Select(entry => (Entry: entry, Day: entry.LocalDate(zone)))
        ...
        var proratedBudget = monthlyBudget is { } budget
            ? CalculateProratedBudget(normalizedStart, normalizedEnd, budget)
            : (double?)null;

        var budgetUsedPercent = proratedBudget is > 0.0
        ...
        if (monthlyBudget is > 0.0 && isSingleMonth)
        {
            ...
            if (averageCreditsPerDay > 0.0 && projectedMonthTotal > monthlyBudget.Value)
            {
                ...
                projectedBudgetRunOutDay = Math.Clamp(runOutDay, 1, month.LengthOfMonth);
            }
        }
        ...
        return new UsageStats
        {
            ...
            HighestSingleEntry = filtered.Count == 0 ? null : filtered.Max(pair => pair.Entry.Amount),
            LatestEntryDate = filtered.Count == 0 ? null : filtered.Max(pair => pair.Day),
