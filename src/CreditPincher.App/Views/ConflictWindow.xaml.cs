...
public partial class ConflictWindow : Window
{
    ...
    private readonly Dictionary<string, Func<string>> _selections = new(StringComparer.OrdinalIgnoreCase);
    ...
    private void BuildFileCards()
    {
        foreach (var file in _conflictedFiles)
        {
            ...
            else if (string.Equals(file, BudgetFile, StringComparison.OrdinalIgnoreCase))
            {
                ...
                AddOption(options, currentFile, groupName, $"Keep the budget set here ({Describe(local)})", true,
                    () => _git.Show(2, currentFile));
                AddOption(options, currentFile, groupName, $"Keep the remote budget ({Describe(remote)})", false,
                    () => _git.Show(3, currentFile));
            }
            else
            {
        ...
        Func<string> resolve)
    {
        ...
        radio.Checked += (_, _) => _selections[file] = resolve;
        container.Children.Add(radio);
    }

    private static string Describe(string value) => value.Length > 0 ? value : "not set";
    ...
    private void OnResolveClick(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            foreach (var file in _conflictedFiles)
            {
                if (_selections.TryGetValue(file, out var resolve))
                {
