...
public sealed class HotKeyManager : IDisposable
{
    ...
    public bool Register(string modifiersText, string keyText)
    {
        ...
        if (!TryParse(modifiersText, keyText, out var modifiers, out var virtualKey))
        {
    ...
    {
        if (_registered && _source is { Handle: var handle } && handle != IntPtr.Zero)
        {
    ...
    public static string Describe(string modifiersText, string keyText) =>
        string.IsNullOrWhiteSpace(modifiersText) ? keyText : $"{modifiersText}+{keyText}";
    ...
    private static bool TryParse(string modifiersText, string keyText, out uint modifiers, out uint virtualKey)
    {
        ...
        foreach (var part in (modifiersText ?? string.Empty).Split('+', StringSplitOptions.RemoveEmptyEntries))
        {
