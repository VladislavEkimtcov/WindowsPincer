...
namespace CreditPincher.App.Tray
{
    ...
    {
        /// <summary>The three ascending bars, matching the application icon.</summary>
        private static readonly float[][] Bars =
        {
            new[] { 52f, 156f },
            new[] { 108f, 112f },
            new[] { 164f, 60f },
        ...
        {
            return Create(status, size, Color.FromArgb(53, 116, 168), out iconHandle);
        }

        /// <summary>
        /// As above, but with the "no budget set" colour supplied by the active theme, so
        /// the icon reads as part of the same scheme as the rest of the app. The other
        /// three states stay green/amber/red: those encode budget status, not decoration.
        /// </summary>
        public static Icon Create(TrayStatus status, int size, Color neutralBackground, out IntPtr iconHandle)
        {
            ...
            using (var bitmap = new Bitmap(size, size))
            {
                using (var graphics = Graphics.FromImage(bitmap))
                {
                    ...
                    using (var background = new SolidBrush(BackgroundFor(status, neutralBackground)))
                    ...
                    using (var barBrush = new SolidBrush(Color.White))
                    {
                        foreach (var bar in Bars)
                        {
                            ...
                            var rectangle = new RectangleF(
                                x * scale, top * scale, 40f * scale, (206f - top) * scale);
        ...
        public static TrayStatus StatusFor(double? monthlyBudget, double totalCredits)
        {
            return StatusFor(monthlyBudget, totalCredits, 80);
        }

        public static TrayStatus StatusFor(double? monthlyBudget, double totalCredits, int warningThresholdPercent)
        {
            ...
            {
                return TrayStatus.Over;
            }

            return usedPercent >= warningThresholdPercent ? TrayStatus.Warning : TrayStatus.Ok;
        }

        private static Color BackgroundFor(TrayStatus status, Color neutral)
        {
            switch (status)
            {
                case TrayStatus.Ok:
                    return Color.FromArgb(46, 125, 50);
                case TrayStatus.Warning:
                    return Color.FromArgb(178, 107, 0);
                case TrayStatus.Over:
